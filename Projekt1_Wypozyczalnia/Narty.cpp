...
Narty::Narty(wxWindow* parent, int ID) : wxDialog(parent, wxID_ANY, "Narty", wxDefaultPosition, wxSize(1000, 740)), ID(ID)
{
	...
	text_nazwa = new wxStaticText(panel_mid, wxID_ANY, "Narty", wxPoint(20, 24));
	text_nazwa->SetFont(text_nazwa->GetFont().Scale(1.6));

	//main_narty--------------------------------------------------------------------------------------------------------
	text_model_nart = new wxStaticText(panel_main2, wxID_ANY, "Model nart", wxPoint(375, 40));
	text_model_nart->SetFont(text_model_nart->GetFont().Scale(1.3));

	choice_model_nart = new wxChoice(panel_main2, wxID_ANY, wxPoint(375, 70), wxSize(150, 30));
	choice_model_nart->Connect(wxEVT_CHOICE, wxCommandEventHandler(Narty::OnChoiceValueChanged), nullptr, this);
	AddModelNart("Head SUPERSHAPE", 100.00);
	AddModelNart("Atomic Redster S7", 150.00);
	AddModelNart("Rossignol PURSUIT", 200.00);
	AddModelNart("Salomon QST Lux", 300.00);
	AddModelNart("Volkl RaceTiger SC", 280.00);
	...
	spin_ctrl_dni->Connect(wxEVT_SPINCTRL, wxSpinEventHandler(Narty::OnSpinCtrlValueChanged), nullptr, this);
	...
	button_dalej->Bind(wxEVT_BUTTON, &Narty::OnButtonDalej, this);

	panel_main1->SetSizerAndFit(sizer_main);
	this->SetSizerAndFit(sizer);
}
void Narty::AddModelNart(wxString firma, double cena)
{
	if (choice_model_nart)
	{
		choice_model_nart->Append(firma);
		prices[firma] = cena;
	}
}

double Narty::cena_wypozyczenia(int value)
{
	double cena_wypozyczenia = 0.0;
	wxString wybrany = choice_model_nart->GetStringSelection();
...
void Narty::OnSpinCtrlValueChanged(wxSpinEvent& event)
{
...
void Narty::OnChoiceValueChanged(wxCommandEvent& event)
{
...
void Narty::OnPodgladModelu(wxCommandEvent& event)
{
	if (bitmap_image)
	{
	...
	wxString numer_zdj = choice_model_nart->GetStringSelection();
...
wxBEGIN_EVENT_TABLE(Narty, wxDialog)
EVT_BUTTON(wxID_ANY, Narty::OnButtonDalej)
wxEND_EVENT_TABLE()